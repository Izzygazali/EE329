...
{
    //set ports 5.4-5.7 as inputs for rows.
    P5 -> SEL0 &= ~(ROW1 + ROW2 + ROW3 + ROW4);
    P5 -> SEL1 &= ~(ROW1 + ROW2 + ROW3 + ROW4);
    P5 -> DIR  &= ~(ROW1 + ROW2 + ROW3 + ROW4);

    P5->IE |= (BIT4 + BIT5 + BIT6 + BIT7);          // Enable interrupts for P5.4 - P5.7
    P5->REN |= (BIT4 + BIT5 + BIT6 + BIT7);         // Add an internal pull down resistor
    P5->IES &= ~(BIT4 + BIT5 + BIT6 + BIT7);        // Select low to high edge Int
    P5->IFG &= ~(BIT4 + BIT5 + BIT6 + BIT7);        // Clear the interrupt flags to ensure system
    ...
    P3 -> OUT  |= (BIT5 + BIT6 + BIT7);             //Drive outputs high

    // Code to run with interrupts
